...
// Model for ProizvodnaJedinica entity
    models.ProizvodnaJedinica = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.proizvodnajedinica/",
        idAttribute: 'idproizvodnajedinica',
        defaults: {
            ukupankapacitetproizvodnje: "",
            naziv: "",
            mesto: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idproizvodnajedinica');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.proizvodnajedinica/';
            }
    ...
    // Collection class for ProizvodnaJedinica entities
    models.ProizvodnaJedinicaCollection = Backbone.Collection.extend({
        model: models.ProizvodnaJedinica,
        url: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.proizvodnajedinica/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProizvodnaJedinicaCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-proizvodnajedinica-list-itemis template identifier for item
                        templateName: '#tpl-proizvodnajedinica-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.proizvodnajedinica = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.proizvodnajedinica,
                    // tpl-proizvodnajedinica-details is template identifier for chosen model element
                    templateName: '#tpl-proizvodnajedinica-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.ProizvodnaJedinica();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-proizvodnajedinica-details is a template identifier for chosen model element
                templateName: '#tpl-proizvodnajedinica-details',
        ...
        getData: function () {
            return {
                 ...
                 * f.e.
                 * idproizvodnajedinica:$('#idproizvodnajedinica').val(),
                 * ....
                 */
            };
        }
