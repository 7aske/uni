...
void reader_form_construct(state_t* state) {
	...
	READER* ptr = (READER*) state->fs.data;
	...
	if (state->fs.ftype == FORM_UPDATE) {
		set_field_buffer(field[0], 0, ptr->username);
		set_field_buffer(field[1], 0, ptr->password);
	}
	...
	mvwprintw(state->win, 4, 10, "Username     :");
	mvwprintw(state->win, 5, 10, "Password     :");
	...
	while ((ch = wgetch(state->win))) {
		switch (ch) {
			...
			case ctrl('x'):
				...
				strncpy(ptr->username, trimws(field_buffer(field[0], 0), sizeof(ptr->username)),
						sizeof(ptr->username));
				strncpy(ptr->password, trimws(field_buffer(field[1], 0), sizeof(ptr->password)),
						sizeof(ptr->password));
