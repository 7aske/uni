...
// Model for NarudzbenicaOtpremnica entity
    models.NarudzbenicaOtpremnica = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.narudzbenicaotpremnica/",
        idAttribute: 'idNarudzbenicaOtpremnica',
        defaults: {
            tipnO: "",
            sifra: ""
        },
        toViewJson: function () {
            ...
            result.displayName = this.get('idNarudzbenicaOtpremnica');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.narudzbenicaotpremnica/';
            }
    ...
    // Collection class for NarudzbenicaOtpremnica entities
    models.NarudzbenicaOtpremnicaCollection = Backbone.Collection.extend({
        model: models.NarudzbenicaOtpremnica,
        url: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.narudzbenicaotpremnica/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.NarudzbenicaOtpremnicaCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-narudzbenicaotpremnica-list-itemis template identifier for item
                        templateName: '#tpl-narudzbenicaotpremnica-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.narudzbenicaotpremnica = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.narudzbenicaotpremnica,
                    // tpl-narudzbenicaotpremnica-details is template identifier for chosen model element
                    templateName: '#tpl-narudzbenicaotpremnica-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.NarudzbenicaOtpremnica();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-narudzbenicaotpremnica-details is a template identifier for chosen model element
                templateName: '#tpl-narudzbenicaotpremnica-details',
        ...
        getData: function () {
            return {
                idNarudzbenicaOtpremnica: $('#idNarudzbenicaOtpremnica').val(),
                tipnO: $('#tipnO').val(),
                sifra: $('#sifra').val()
            };
        }
