...
// Model for Kolicinaproizvoda entity
    models.Kolicinaproizvoda = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.kolicinaproizvoda/",
        idAttribute: 'idkolicinaproizvoda',
        defaults: {
            kolicina: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idkolicinaproizvoda');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.kolicinaproizvoda/';
            }
    ...
    // Collection class for Kolicinaproizvoda entities
    models.KolicinaproizvodaCollection = Backbone.Collection.extend({
        model: models.Kolicinaproizvoda,
        url: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.kolicinaproizvoda/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.KolicinaproizvodaCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-kolicinaproizvoda-list-itemis template identifier for item
                        templateName: '#tpl-kolicinaproizvoda-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.kolicinaproizvoda = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.kolicinaproizvoda,
                    // tpl-kolicinaproizvoda-details is template identifier for chosen model element
                    templateName: '#tpl-kolicinaproizvoda-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Kolicinaproizvoda();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-kolicinaproizvoda-details is a template identifier for chosen model element
                templateName: '#tpl-kolicinaproizvoda-details',
        ...
        getData: function () {
            return {
                idkolicinaproizvoda: $('#idkolicinaproizvoda').val(),
