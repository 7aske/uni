...
void book_specimen_form_construct(state_t* state) {
	...
	BOOK_SPECIMEN* ptr = (BOOK_SPECIMEN*) state->fs.data;
	...
	if (state->fs.ftype == FORM_UPDATE) {
		...
		set_field_buffer(field[2], 0, ptr->book_serial);
	}
	...
	mvwprintw(state->win, 6, 10, "Serial       :");
	...
	while ((ch = wgetch(state->win))) {
		switch (ch) {
			...
			case ctrl('x'):
				...
				id1 = (int) strtol(trimws(buf1, BUFLEN), NULL, 10);
				id2 = (int) strtol(trimws(buf2, BUFLEN), NULL, 10);

				strncpy(ptr->book_serial, trimws(field_buffer(field[2], 0), sizeof(ptr->book_serial)),
						sizeof(ptr->book_serial));


				if (state->fs.ftype == FORM_CREATE) {
					action = type_insert_action(state->fs.type);
					ptr->id_book_specimen = 0;
