...
// Model for Skladiste entity
    models.Skladiste = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.skladiste/",
        idAttribute: 'idskladiste',
        defaults: {
            ukupnikapacitetzaliha: "",
            naziv: "",
            mesto: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idskladiste');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.skladiste/';
            }
    ...
    // Collection class for Skladiste entities
    models.SkladisteCollection = Backbone.Collection.extend({
        model: models.Skladiste,
        url: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.skladiste/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.SkladisteCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-skladiste-list-itemis template identifier for item
                        templateName: '#tpl-skladiste-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.skladiste = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.skladiste,
                    // tpl-skladiste-details is template identifier for chosen model element
                    templateName: '#tpl-skladiste-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Skladiste();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-skladiste-details is a template identifier for chosen model element
                templateName: '#tpl-skladiste-details',
        ...
        getData: function () {
            return {
                idskladiste: $('#idskladiste').val(),
                ukupnikapacitetzaliha: $('#ukupnikapacitetzaliha').val(),
                naziv: $('#naziv').val(),
                mesto: $('#mesto').val()
            };
        }
