...
void library_form_construct(state_t* state) {
	...
	if (state->fs.ftype == FORM_UPDATE) {
		snprintf(buf1, BUFLEN, "%d", ptr->address->id_address);
	...
	mvwprintw(state->win, 4, 10, "Address      :");
	mvwprintw(state->win, 5, 10, "Name         :");
	...
	while ((ch = wgetch(state->win))) {
		switch (ch) {
			...
			case ctrl('x'):
				...
				strncpy(ptr->name, trimws(field_buffer(field[1], 0), sizeof(ptr->name)), sizeof(ptr->name));
				...
				ptr->address = address_find_by_id(state->conn, id1);
