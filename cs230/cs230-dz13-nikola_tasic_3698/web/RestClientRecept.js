...
// Model for Recept entity
    models.Recept = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.recept/",
        idAttribute: 'idrecept',
        defaults: {
            tekstrecepta: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idrecept');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.recept/';
            }
    ...
    // Collection class for Recept entities
    models.ReceptCollection = Backbone.Collection.extend({
        model: models.Recept,
        url: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.recept/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ReceptCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-recept-list-itemis template identifier for item
                        templateName: '#tpl-recept-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.recept = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.recept,
                    // tpl-recept-details is template identifier for chosen model element
                    templateName: '#tpl-recept-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Recept();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-recept-details is a template identifier for chosen model element
                templateName: '#tpl-recept-details',
