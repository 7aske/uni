...
unsigned long z09probefunc(int nmemb, unsigned long hash) {
	...
	c++;
	printf("Hash position %lu is taken ", hash);
...
int main() {
	...
	htable_t* emptable = htable_new(sizeof(unsigned long), sizeof(struct employee));
	emptable->probefunc = z09probefunc;
	printf("Hashing employee: {%lu, '%s', '%s'} into %lu\n", e1.id, e1.fname, e1.lname, _hashfunc(emptable->nmemb, &e1.id, sizeof(struct employee)));
	htable_add(emptable, &e1.id ,&e1);
	printf("Hashing employee: {%lu, '%s', '%s'} into %lu\n", e2.id, e2.fname, e2.lname, _hashfunc(emptable->nmemb, &e2.id, sizeof(struct employee)));
	htable_add(emptable, &e2.id, &e2);
	printf("Hashing employee: {%lu, '%s', '%s'} into %lu\n", e3.id, e3.fname, e3.lname, _hashfunc(emptable->nmemb, &e3.id, sizeof(struct employee)));
	htable_add(emptable, &e3.id, &e3);
	printf("Hashing employee: {%lu, '%s', '%s'} into %lu\n", e4.id, e4.fname, e4.lname, _hashfunc(emptable->nmemb, &e4.id, sizeof(struct employee)));
	htable_add(emptable, &e4.id, &e4);
	printf("Hashing employee: {%lu, '%s', '%s'} into %lu\n", e5.id, e5.fname, e5.lname, _hashfunc(emptable->nmemb, &e5.id, sizeof(struct employee)));
	htable_add(emptable, &e5.id, &e5);
	printf("Hashing employee: {%lu, '%s', '%s'} into %lu\n", e6.id, e6.fname, e6.lname, _hashfunc(emptable->nmemb, &e6.id, sizeof(struct employee)));
	htable_add(emptable, &e6.id, &e6);
