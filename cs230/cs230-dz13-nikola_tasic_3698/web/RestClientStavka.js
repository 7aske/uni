...
// Model for Stavka entity
    models.Stavka = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.stavka/",
        idAttribute: 'idstavka',
        defaults: {
            kolicina: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idstavka');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.stavka/';
            }
    ...
    // Collection class for Stavka entities
    models.StavkaCollection = Backbone.Collection.extend({
        model: models.Stavka,
        url: "http://localhost:8080/CS230-DZ13-David_Ivanovic-3585/webresources/entity.stavka/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.StavkaCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-stavka-list-itemis template identifier for item
                        templateName: '#tpl-stavka-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.stavka = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.stavka,
                    // tpl-stavka-details is template identifier for chosen model element
                    templateName: '#tpl-stavka-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Stavka();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-stavka-details is a template identifier for chosen model element
                templateName: '#tpl-stavka-details',
